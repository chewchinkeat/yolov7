...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, PatchEmbed]:  # Adjusted to include PatchEmbed
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m is PatchEmbed and len(args) < 4:
                args.append(None)  # Adding None for norm_layer if not provided
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
                 ...
                 SwinTransformer2Block, ST2CSPA, ST2CSPB, ST2CSPC,BasicStage,PatchEmbed_FasterNet,PatchMerging_FasterNet,C3,SPDConv]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)
